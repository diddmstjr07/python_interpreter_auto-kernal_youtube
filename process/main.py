...
def directory_clear():
    try:
        os.remove(f"{store}/content_result.json")
        os.remove(f"{store}/content_result.txt")
    except:
...
class arrange:
    ...
    show_result()
    downloadment()
    delete_part_files(store[0] + "/videos/youtube/")
    link_address = url_input()
    reset()
    download_video(link_address, store[0] + "/videos/youtube/")
    directory_clear()
    print("\033[1m\033[92m" + "\n\n✅ Download Completed ✅\n" + "\033[0m")
